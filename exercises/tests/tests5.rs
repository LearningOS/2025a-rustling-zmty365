...
/// The memory must not be accessed by other references during this operation.
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller guarantees that the address is valid, properly aligned,
    // points to a mutable u32, and has exclusive access to the memory.
    // We're converting the usize back to a mutable pointer and dereferencing it
    // to modify the value, which is safe under these conditions.
    unsafe {
